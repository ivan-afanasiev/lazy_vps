...
4. **SSL/TLS mode**: Cloudflare → SSL/TLS → Overview → set to **"Flexible"**. This is the mode that matches our origin: Cloudflare speaks HTTPS to the *visitor*, but plain HTTP to *us* (Xray's WS endpoint on `:8080` doesn't terminate TLS — that's what we want CF to do). Setting "Full" or "Full (strict)" here gives `525 SSL handshake failed`. Edge Certificates → ensure **"Always Use HTTPS"** is **off** (it interferes with WS upgrades on some configs).
