...
bot-install: check-bot-env ## Install/upgrade the bot on an already-deployed VPS (no reboot, no Xray/Telemt impact)
	...
	TAILSCALE_ENABLED=$$(cd $(TF_DIR) && terraform output -raw tailscale_enabled 2>/dev/null || echo false); \
	[ "$$TAILSCALE_ENABLED" = "true" ] && TAILSCALE_ENV=true || TAILSCALE_ENV=; \
	...
	ssh -o StrictHostKeyChecking=no ubuntu@$$HOST "sudo mkdir -p /opt/lazy-vps-bot && \
		...
		sudo env \
			...
			TAILSCALE_ENABLED='$$TAILSCALE_ENV' \
